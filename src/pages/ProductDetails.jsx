...
export default function ProductDetails() {
  ...
  return (
    <div className="flex flex-col pt-10 bg-palette-light justify-center items-center md:flex-row md:items-start space-y-8 md:space-y-0 md:space-x-4 lg:space-x-8 max-w-6xl w-11/12 mx-auto">
      ...
      <button type="button" onClick={() => navigate('/cart')} className="relative" aria-label="cart">
        <FontAwesomeIcon className="text-palette-primary w-8 m-auto fa-5x" icon={faShoppingCart} />
        {
                !cart
                  ...
                  : (
                    <div
                      className="absolute top-6 right-7 text-xs bg-yellow-300 text-gray-900 font-semibold rounded-full py-1 px-2 transform translate-x-10 -translate-y-3"
                    >
                      {cart.length}
                    </div>
                  )
